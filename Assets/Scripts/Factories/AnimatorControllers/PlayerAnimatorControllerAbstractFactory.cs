...
{
    public abstract class PlayerAnimatorControllerAbstractFactory
    {
        protected string _movementAnimation;
        protected const string playerControllersPath = "Assets/Animators/Player/";
        List<AnimatorStateMachine> AnimatorSubStateMachines { get; set; }
        AnimatorStateMachine rootStateMachine;
        IEnumerable<DirectionEnum> directionsEnumValues;

        //Controller Parameters
        const string combatActiveParam = "IsCombatActive";
        const string walkingParam = "Walking";
        const string runningParam = "Running";
        const string directionParam = "Direction";
        const string attackSequenceParam = "AttackSequence";

        //SubStateMachines
        const string subStateIdleName = "Idle";
        const string subStateWalkingName = "Walking";
        const string subStateCombatIdleStanceName = "CombatIdleStance";
        const string subStateCombatWalkingStanceName = "CombatWalkingStance";
        const string subStateRunningName = "Running";
        const string subStateAttackingName = "Attacking";

        //Attack States
        const string attackSequence_1 = "AttackSequence_1";
        const string attackSequence_2 = "AttackSequence_2";
        const string attackSequence_3 = "AttackSequence_3";

        string[] mainSubStates = new[] { subStateIdleName, subStateWalkingName, subStateCombatIdleStanceName, subStateCombatWalkingStanceName, subStateRunningName, subStateAttackingName };
        string[] attackStates = new[] { attackSequence_1, attackSequence_2, attackSequence_3 };
        ...
        protected void GenerateAnimations(string controllerName, List<Motion> motionClips)
        {
            ...
            string pathWithFileName = string.Format("{0}{1}PlayerAnimatorController.controller", playerControllersPath, controllerName);
        ...
        protected Motion[] GetMotionClips(string movement, string bodyPart)
        {
            string animationResourcesPath = string.Format("Animation/{0}/Player/{1}", movement, bodyPart);
        ...
        void SetAnimationSpeed(AnimatorState state, float speed)
        {
            state.speed = speed;
        }
        ...
        void CreateInternalAttackingTransitionBetweenStates(AnimatorStateMachine attackingSubStateMachine)
        {
            ...
            for (int i = 0; i < attackingSubStateMachine.states.Length; i++) //currentState
            {
                for (int j = 0; j < attackingSubStateMachine.states.Length; j++) //otherState
                {
                    ...
                    var otherState = attackingSubStateMachine.states[j];
            ...
            foreach (var stateMachine in AnimatorSubStateMachines)
            {
                ...
                for (int i = 0; i < stateMachine.states.Length; i++) //currentState
                {
                    for (int j = 0; j < stateMachine.states.Length; j++) //otherState
                    {
                        ...
                        var otherState = stateMachine.states[j];
        ...
        private void TransitionBetweenIdleRunning(AnimatorStateMachine idleSubStateMachine, AnimatorStateMachine runningSubStateMachine)
        {
            ...
            transitionConditions[0] = new TransitionConditions() { conditionMode = AnimatorConditionMode.IfNot, parameterName = walkingParam };
            transitionConditions[1] = new TransitionConditions() { conditionMode = AnimatorConditionMode.IfNot, parameterName = runningParam };
            CreateOneWayTransitionCondition(idleSubStateMachine, runningSubStateMachine, transitionConditions);
        }
        #endregion
